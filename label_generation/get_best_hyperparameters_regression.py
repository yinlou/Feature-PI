...
def get_n_estimators_by_cv(params, dtrain):
    ...
    best_score = np.min(validation_summary["valid %s-mean" % metric])
...
def gridsearch_tuning(X, y):
    ...
    best_params = cv_results[0][0]
    best_params['n_estimators'] = cv_results[0][1]
    best_score = cv_results[0][2]
...
def run_tuning(data_path):
    ...
    logger.log("Time spent tuning:")
    logger.log((datetime.datetime.now() - start))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--metric", help="metric for scoring function", type=str, default='mape')
    ...
    logger = Logger(myPath, 'get_best_hyperparameters_regression_s%d' % seed)
