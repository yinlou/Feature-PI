...
def labeling_by_permutation(best_params, data, label, data_path):
    ...
    for i in range(3):
        ...
        logger.log("Round %d." % i)
        logger.log("Start permutation.")

        cv_fold = 0
        n_repeats = 1
        for train_index, test_index in skf.split(X, y):
            cv_fold += 1
            logger.log("CV fold %d." % cv_fold)
    ...
    with open(os.path.join(data_path, 'permutation_importance_s%d.json' % seed), 'w') as f:
...
def run_labeling(data_path):
    # if 'permutation_importance_s%d.json' % seed in os.listdir(data_path):
    #   return 0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--metric", help="metric for scoring function", type=str, default='roc_auc')
    ...
    logger = Logger(myPath, 'generate_labels_binary_classification_%s_s%d' % (metric, seed))
